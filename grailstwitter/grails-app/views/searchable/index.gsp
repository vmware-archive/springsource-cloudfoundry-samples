    ...
    <g:if test="${haveResults}">
      <div class="results">
        <g:each var="result" in="${searchResult.results}" status="index">
          <div class="result">
            <g:if test="${result instanceof Person}">
              <div class="name">${result.realName}</div>
              <div class="displayLink"><g:link id="${result.id}" action="follow" controller="status">follow</g:link></div>
            </g:if>
            <g:else>
              <g:render template="/status/statusMessages" model="[statusMessage: result]"/>
            </g:else>
          </div>
        </g:each>
      </div>
